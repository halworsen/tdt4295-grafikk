...
class RecDrawer(coordWidth: Int = 16) extends Module {
  ...
  when(
    ld1.io.done === true.B && ld2.io.done === true.B && ld3.io.done === true.B && ld4.io.done === true.B
  ) {
    io.done := true.B
    // Set up ld4
  }.elsewhen(
    ld1.io.done === true.B && ld2.io.done === true.B && ld3.io.done === true.B
  ) {
    ld4.io.xs := io.thirdX
    ld4.io.ys := io.thirdY
    ld4.io.xe := io.firstX
    ...
    io.writeEnable := ld4.io.writeEnable
    io.writeX := ld4.io.writeX
    io.writeY := ld4.io.writeY
    io.writeVal := ld4.io.writeVal
