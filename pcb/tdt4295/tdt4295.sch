EESchema Schematic File Version 4
EELAYER 30 0
EELAYER END
$Descr A3 16535 11693
encoding utf-8
Sheet 1 2
Title ""
Date ""
Rev ""
Comp ""
Comment1 ""
Comment2 ""
Comment3 ""
Comment4 ""
$EndDescr
$Sheet
S 750  950  1150 1000
U 613AE19D
F0 "Power_components" 50
F1 "Power_Components.sch" 50
F2 "USB_D+" B R 1900 1050 50 
F3 "USB_D-" B R 1900 1150 50 
F4 "+5V" O R 1900 1450 50 
F5 "+3V3" O R 1900 1550 50 
F6 "+3V3A" O R 1900 1650 50 
F7 "+1V8" O R 1900 1750 50 
F8 "+1V0" O R 1900 1850 50 
$EndSheet
$Comp
L EFM32GG12B810F1024GQ64:EFM32GG12B810F1024GQ64 U?
U 1 1 614304EC
P 6850 1600
F 0 "U?" H 7850 1987 60  0000 C CNN
F 1 "EFM32GG12B810F1024GQ64" H 7850 1881 60  0000 C CNN
F 2 "efm32gg-footprint:EFM32GG12B810F1024GQ64" H 7850 1840 60  0001 C CNN
F 3 "${KIPRJMOD}/docs/datasheets/efm32gg12-datasheet.pdf" H 7850 1881 60  0001 C CNN
	1    6850 1600
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 61449541
P 6150 2350
F 0 "#PWR?" H 6150 2100 50  0001 C CNN
F 1 "GND" H 6155 2177 50  0000 C CNN
F 2 "" H 6150 2350 50  0001 C CNN
F 3 "" H 6150 2350 50  0001 C CNN
	1    6150 2350
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR?
U 1 1 61455354
P 2000 1400
F 0 "#PWR?" H 2000 1250 50  0001 C CNN
F 1 "+5V" H 2015 1573 50  0000 C CNN
F 2 "" H 2000 1400 50  0001 C CNN
F 3 "" H 2000 1400 50  0001 C CNN
	1    2000 1400
	1    0    0    -1  
$EndComp
Wire Wire Line
	2000 1400 2000 1450
Wire Wire Line
	2000 1450 1900 1450
$Comp
L power:+3V3 #PWR?
U 1 1 61455D75
P 2200 1400
F 0 "#PWR?" H 2200 1250 50  0001 C CNN
F 1 "+3V3" H 2215 1573 50  0000 C CNN
F 2 "" H 2200 1400 50  0001 C CNN
F 3 "" H 2200 1400 50  0001 C CNN
	1    2200 1400
	1    0    0    -1  
$EndComp
$Comp
L tdt4295:+3V3A #PWR?
U 1 1 614565B9
P 2450 1400
F 0 "#PWR?" H 2450 1250 50  0001 C CNN
F 1 "+3V3A" H 2465 1573 50  0000 C CNN
F 2 "" H 2450 1400 50  0001 C CNN
F 3 "" H 2450 1400 50  0001 C CNN
	1    2450 1400
	1    0    0    -1  
$EndComp
$Comp
L power:+1V8 #PWR?
U 1 1 61456C3B
P 2700 1400
F 0 "#PWR?" H 2700 1250 50  0001 C CNN
F 1 "+1V8" H 2715 1573 50  0000 C CNN
F 2 "" H 2700 1400 50  0001 C CNN
F 3 "" H 2700 1400 50  0001 C CNN
	1    2700 1400
	1    0    0    -1  
$EndComp
$Comp
L power:+1V0 #PWR?
U 1 1 614570F0
P 2950 1400
F 0 "#PWR?" H 2950 1250 50  0001 C CNN
F 1 "+1V0" H 2965 1573 50  0000 C CNN
F 2 "" H 2950 1400 50  0001 C CNN
F 3 "" H 2950 1400 50  0001 C CNN
	1    2950 1400
	1    0    0    -1  
$EndComp
Wire Wire Line
	1900 1550 2200 1550
Wire Wire Line
	2200 1550 2200 1400
Wire Wire Line
	2450 1400 2450 1650
Wire Wire Line
	2450 1650 1900 1650
Wire Wire Line
	1900 1750 2700 1750
Wire Wire Line
	2700 1750 2700 1400
Wire Wire Line
	2950 1400 2950 1850
Wire Wire Line
	2950 1850 1900 1850
$Comp
L power:GND #PWR?
U 1 1 614604CD
P 9500 4500
F 0 "#PWR?" H 9500 4250 50  0001 C CNN
F 1 "GND" H 9505 4327 50  0000 C CNN
F 2 "" H 9500 4500 50  0001 C CNN
F 3 "" H 9500 4500 50  0001 C CNN
	1    9500 4500
	1    0    0    -1  
$EndComp
$Comp
L power:+3V3 #PWR?
U 1 1 6147F4DA
P 6150 2150
F 0 "#PWR?" H 6150 2000 50  0001 C CNN
F 1 "+3V3" H 6165 2323 50  0000 C CNN
F 2 "" H 6150 2150 50  0001 C CNN
F 3 "" H 6150 2150 50  0001 C CNN
	1    6150 2150
	1    0    0    -1  
$EndComp
$Comp
L power:+3V3 #PWR?
U 1 1 61483FDB
P 9800 2300
F 0 "#PWR?" H 9800 2150 50  0001 C CNN
F 1 "+3V3" H 9815 2473 50  0000 C CNN
F 2 "" H 9800 2300 50  0001 C CNN
F 3 "" H 9800 2300 50  0001 C CNN
	1    9800 2300
	1    0    0    -1  
$EndComp
Wire Wire Line
	4400 3600 4650 3600
Wire Wire Line
	4400 3600 4400 3700
Wire Wire Line
	4800 4050 5100 4050
Wire Wire Line
	4650 4600 4650 4500
Wire Wire Line
	4650 4250 4650 4500
Connection ~ 4650 4500
Wire Wire Line
	4200 4500 4200 4350
Wire Wire Line
	4650 4500 4200 4500
Wire Wire Line
	5100 4500 5100 4350
Wire Wire Line
	4650 4500 5100 4500
Wire Wire Line
	4650 3850 4650 3600
$Comp
L power:GND #PWR?
U 1 1 6146F7C4
P 4400 3700
F 0 "#PWR?" H 4400 3450 50  0001 C CNN
F 1 "GND" H 4405 3527 50  0000 C CNN
F 2 "" H 4400 3700 50  0001 C CNN
F 3 "" H 4400 3700 50  0001 C CNN
	1    4400 3700
	1    0    0    -1  
$EndComp
$Comp
L Device:C C?
U 1 1 6146ECCE
P 5100 4200
F 0 "C?" H 5215 4246 50  0000 L CNN
F 1 "12pF" H 5215 4155 50  0000 L CNN
F 2 "" H 5138 4050 50  0001 C CNN
F 3 "~" H 5100 4200 50  0001 C CNN
	1    5100 4200
	1    0    0    -1  
$EndComp
$Comp
L Device:C C?
U 1 1 6146E505
P 4200 4200
F 0 "C?" H 4315 4246 50  0000 L CNN
F 1 "12pF" H 4315 4155 50  0000 L CNN
F 2 "" H 4238 4050 50  0001 C CNN
F 3 "~" H 4200 4200 50  0001 C CNN
	1    4200 4200
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 6146D608
P 4650 4600
F 0 "#PWR?" H 4650 4350 50  0001 C CNN
F 1 "GND" H 4655 4427 50  0000 C CNN
F 2 "" H 4650 4600 50  0001 C CNN
F 3 "" H 4650 4600 50  0001 C CNN
	1    4650 4600
	1    0    0    -1  
$EndComp
$Comp
L Device:Crystal_GND24 Y?
U 1 1 6146C42D
P 4650 4050
F 0 "Y?" H 4750 4200 50  0000 L CNN
F 1 "48MHz" H 4750 3950 50  0000 L CNN
F 2 "" H 4650 4050 50  0001 C CNN
F 3 "~" H 4650 4050 50  0001 C CNN
	1    4650 4050
	1    0    0    -1  
$EndComp
$Comp
L power:+3V3 #PWR?
U 1 1 6149B77D
P 5500 3800
F 0 "#PWR?" H 5500 3650 50  0001 C CNN
F 1 "+3V3" H 5515 3973 50  0000 C CNN
F 2 "" H 5500 3800 50  0001 C CNN
F 3 "" H 5500 3800 50  0001 C CNN
	1    5500 3800
	1    0    0    -1  
$EndComp
$Comp
L tdt4295:+3V3A #PWR?
U 1 1 6149CE44
P 5800 3800
F 0 "#PWR?" H 5800 3650 50  0001 C CNN
F 1 "+3V3A" H 5815 3973 50  0000 C CNN
F 2 "" H 5800 3800 50  0001 C CNN
F 3 "" H 5800 3800 50  0001 C CNN
	1    5800 3800
	1    0    0    -1  
$EndComp
Text GLabel 2150 1050 2    50   BiDi ~ 0
USB_D+
Text GLabel 2150 1150 2    50   BiDi ~ 0
USB_D-
Wire Wire Line
	2150 1150 1900 1150
Wire Wire Line
	2150 1050 1900 1050
Text GLabel 8950 3300 2    50   BiDi ~ 0
USB_D-
Text GLabel 8950 3200 2    50   BiDi ~ 0
USB_D+
$Comp
L power:+3V3 #PWR?
U 1 1 614B5790
P 11900 1750
F 0 "#PWR?" H 11900 1600 50  0001 C CNN
F 1 "+3V3" H 11915 1923 50  0000 C CNN
F 2 "" H 11900 1750 50  0001 C CNN
F 3 "" H 11900 1750 50  0001 C CNN
	1    11900 1750
	1    0    0    -1  
$EndComp
Wire Wire Line
	11750 1750 11750 1850
Wire Wire Line
	11750 1850 11900 1850
Wire Wire Line
	11900 1850 11900 1750
NoConn ~ 11650 1750
NoConn ~ 11550 1750
NoConn ~ 11250 1750
NoConn ~ 10950 1750
NoConn ~ 10850 1750
$Comp
L power:GND #PWR?
U 1 1 614CA866
P 10700 1450
F 0 "#PWR?" H 10700 1200 50  0001 C CNN
F 1 "GND" H 10705 1277 50  0000 C CNN
F 2 "" H 10700 1450 50  0001 C CNN
F 3 "" H 10700 1450 50  0001 C CNN
	1    10700 1450
	1    0    0    -1  
$EndComp
$Comp
L Connector_Generic:Conn_02x10_Odd_Even J?
U 1 1 614A6418
P 11350 1550
F 0 "J?" V 11550 2050 50  0000 L CNN
F 1 "MCU Debug Header" V 11450 2050 50  0000 L CNN
F 2 "" H 11350 1550 50  0001 C CNN
F 3 "~" H 11350 1550 50  0001 C CNN
	1    11350 1550
	0    1    -1   0   
$EndComp
Wire Wire Line
	11650 1250 11550 1250
Wire Wire Line
	11450 1250 11550 1250
Connection ~ 11550 1250
Wire Wire Line
	11450 1250 11350 1250
Connection ~ 11450 1250
Wire Wire Line
	11250 1250 11350 1250
Connection ~ 11350 1250
Wire Wire Line
	11250 1250 11150 1250
Connection ~ 11250 1250
Wire Wire Line
	11150 1250 11050 1250
Connection ~ 11150 1250
Wire Wire Line
	11050 1250 10950 1250
Connection ~ 11050 1250
Wire Wire Line
	10950 1250 10850 1250
Connection ~ 10950 1250
Wire Wire Line
	10850 1250 10700 1250
Wire Wire Line
	10700 1250 10700 1450
Connection ~ 10850 1250
$Comp
L power:GND #PWR?
U 1 1 614E1F63
P 9600 2450
F 0 "#PWR?" H 9600 2200 50  0001 C CNN
F 1 "GND" H 9605 2277 50  0000 C CNN
F 2 "" H 9600 2450 50  0001 C CNN
F 3 "" H 9600 2450 50  0001 C CNN
	1    9600 2450
	1    0    0    -1  
$EndComp
Wire Wire Line
	5800 3900 5800 3800
Wire Wire Line
	5500 4200 5500 3800
$Comp
L power:GND #PWR?
U 1 1 614EFE21
P 6050 3650
F 0 "#PWR?" H 6050 3400 50  0001 C CNN
F 1 "GND" H 6055 3477 50  0000 C CNN
F 2 "" H 6050 3650 50  0001 C CNN
F 3 "" H 6050 3650 50  0001 C CNN
	1    6050 3650
	1    0    0    -1  
$EndComp
$Comp
L power:+3V3 #PWR?
U 1 1 614F48B8
P 9250 4250
F 0 "#PWR?" H 9250 4100 50  0001 C CNN
F 1 "+3V3" H 9265 4423 50  0000 C CNN
F 2 "" H 9250 4250 50  0001 C CNN
F 3 "" H 9250 4250 50  0001 C CNN
	1    9250 4250
	1    0    0    -1  
$EndComp
Wire Wire Line
	9100 4200 9100 4100
$Comp
L power:+5V #PWR?
U 1 1 614FB38E
P 9950 2700
F 0 "#PWR?" H 9950 2550 50  0001 C CNN
F 1 "+5V" H 9965 2873 50  0000 C CNN
F 2 "" H 9950 2700 50  0001 C CNN
F 3 "" H 9950 2700 50  0001 C CNN
	1    9950 2700
	1    0    0    -1  
$EndComp
$Comp
L power:+5V #PWR?
U 1 1 614FDBE7
P 9550 3650
F 0 "#PWR?" H 9550 3500 50  0001 C CNN
F 1 "+5V" H 9565 3823 50  0000 C CNN
F 2 "" H 9550 3650 50  0001 C CNN
F 3 "" H 9550 3650 50  0001 C CNN
	1    9550 3650
	1    0    0    -1  
$EndComp
$Comp
L Device:C C?
U 1 1 61500553
P 9950 4150
F 0 "C?" H 10065 4196 50  0000 L CNN
F 1 "4.7uF" H 10065 4105 50  0000 L CNN
F 2 "" H 9988 4000 50  0001 C CNN
F 3 "~" H 9950 4150 50  0001 C CNN
	1    9950 4150
	1    0    0    -1  
$EndComp
Text GLabel 5100 3800 1    50   UnSpc ~ 0
48MHZ_P3
Text GLabel 4200 3800 1    50   UnSpc ~ 0
48MHZ_P1
Wire Wire Line
	4200 3800 4200 4050
Wire Wire Line
	5100 3800 5100 4050
Connection ~ 5100 4050
Wire Wire Line
	4200 4050 4500 4050
Connection ~ 4200 4050
Text GLabel 6750 4000 0    50   UnSpc ~ 0
48MHZ_P3
Text GLabel 6750 4100 0    50   UnSpc ~ 0
48MHZ_P1
Wire Wire Line
	9950 2800 9950 2700
Text GLabel 8950 2900 2    50   Output ~ 0
DBG_SWO
Text GLabel 8950 3000 2    50   BiDi ~ 0
DBG_SWDIO
Text GLabel 8950 3100 2    50   Output ~ 0
DBG_SWCLK
Text GLabel 11150 1850 3    50   Input ~ 0
DBG_SWO
Text GLabel 11350 1850 3    50   Input ~ 0
DBG_SWCLK
Text GLabel 11450 1850 3    50   BiDi ~ 0
DBG_SWDIO
Text GLabel 6750 1600 0    50   Output ~ 0
QSPI0_CS0
Text GLabel 6750 1700 0    50   Output ~ 0
QSPI0_CS1
Text GLabel 6750 1800 0    50   Output ~ 0
QSPI0_DQ0
Text GLabel 6750 1900 0    50   Output ~ 0
QSPI0_DQ1
Text GLabel 6750 2000 0    50   Output ~ 0
QSPI0_DQ2
Wire Wire Line
	6150 2200 6150 2150
Wire Wire Line
	6150 2200 6850 2200
Wire Wire Line
	6150 2300 6150 2350
Wire Wire Line
	6150 2300 6850 2300
Text GLabel 6750 2100 0    50   Output ~ 0
QSPI0_DQ3
Text GLabel 6750 2400 0    50   Input ~ 0
QSPI0_DQ4
Text GLabel 6750 2500 0    50   Input ~ 0
QSPI0_DQ5
Text GLabel 6750 2600 0    50   Input ~ 0
QSPI0_DQ6
Text GLabel 6750 2700 0    50   Input ~ 0
QSPI0_DQ7
Text GLabel 8950 1600 2    50   UnSpc ~ 0
QSPI0_DQS
Text GLabel 8950 1700 2    50   Output ~ 0
QSPI0_SCLK
Connection ~ 9500 4400
Wire Wire Line
	9500 4400 9500 4500
Wire Wire Line
	9100 4200 9100 4350
Wire Wire Line
	9100 4350 9250 4350
Wire Wire Line
	9250 4350 9250 4250
Connection ~ 9100 4200
Wire Wire Line
	9950 4300 9950 4400
Wire Wire Line
	9950 4400 9500 4400
Text GLabel 6750 4700 0    50   BiDi ~ 0
US3_CLK
Text GLabel 8950 3900 2    50   BiDi ~ 0
US3_CS
Text GLabel 8950 3800 2    50   Input ~ 0
US3_CTS
Wire Wire Line
	9450 3500 9450 3700
Wire Wire Line
	9450 3700 9550 3700
Wire Wire Line
	9550 3700 9550 3650
Text GLabel 6750 3400 0    50   Output ~ 0
US3_RTS
Text GLabel 6750 3000 0    50   Input ~ 0
US3_RX
Text GLabel 8950 3700 2    50   Output ~ 0
US3_TX
$Comp
L Connector_Generic:Conn_02x10_Odd_Even J?
U 1 1 6148061B
P 11350 3350
F 0 "J?" V 11550 3850 50  0000 L CNN
F 1 "MCU GPIO Header" V 11450 3850 50  0000 L CNN
F 2 "" H 11350 3350 50  0001 C CNN
F 3 "~" H 11350 3350 50  0001 C CNN
	1    11350 3350
	0    1    -1   0   
$EndComp
NoConn ~ 11750 1250
$Comp
L power:+3V3 #PWR?
U 1 1 6149A7FB
P 11850 2850
F 0 "#PWR?" H 11850 2700 50  0001 C CNN
F 1 "+3V3" H 11865 3023 50  0000 C CNN
F 2 "" H 11850 2850 50  0001 C CNN
F 3 "" H 11850 2850 50  0001 C CNN
	1    11850 2850
	1    0    0    -1  
$EndComp
$Comp
L power:GND #PWR?
U 1 1 6149AFB1
P 11850 3750
F 0 "#PWR?" H 11850 3500 50  0001 C CNN
F 1 "GND" H 11855 3577 50  0000 C CNN
F 2 "" H 11850 3750 50  0001 C CNN
F 3 "" H 11850 3750 50  0001 C CNN
	1    11850 3750
	1    0    0    -1  
$EndComp
Wire Wire Line
	9950 3400 9950 4000
Wire Wire Line
	11750 3650 11750 3550
Wire Wire Line
	11750 2950 11750 3050
Text GLabel 11150 4500 0    50   BiDi ~ 0
SDIO_DAT3
Text GLabel 11150 4600 0    50   Input ~ 0
SDIO_CMD
Text GLabel 11150 5100 0    50   BiDi ~ 0
SDIO_DAT0
Text GLabel 11150 4400 0    50   BiDi ~ 0
SDIO_DAT2
Text GLabel 11150 5200 0    50   BiDi ~ 0
SDIO_DAT1
Text GLabel 11150 4900 0    50   Input ~ 0
SDIO_CLK
$Comp
L power:GND #PWR?
U 1 1 614E24F0
P 10450 4550
F 0 "#PWR?" H 10450 4300 50  0001 C CNN
F 1 "GND" H 10455 4377 50  0000 C CNN
F 2 "" H 10450 4550 50  0001 C CNN
F 3 "" H 10450 4550 50  0001 C CNN
	1    10450 4550
	1    0    0    -1  
$EndComp
$Comp
L power:+3V3 #PWR?
U 1 1 614E72A4
P 10300 4700
F 0 "#PWR?" H 10300 4550 50  0001 C CNN
F 1 "+3V3" H 10315 4873 50  0000 C CNN
F 2 "" H 10300 4700 50  0001 C CNN
F 3 "" H 10300 4700 50  0001 C CNN
	1    10300 4700
	1    0    0    -1  
$EndComp
Wire Wire Line
	10300 4800 10300 4700
Text GLabel 6750 2800 0    50   BiDi ~ 0
SDIO_CD
Text GLabel 8950 1800 2    50   Output ~ 0
SDIO_CLK
Text GLabel 8950 1900 2    50   Output ~ 0
SDIO_CMD
Text GLabel 8950 2000 2    50   BiDi ~ 0
SDIO_DAT0
Text GLabel 8950 2100 2    50   BiDi ~ 0
SDIO_DAT1
Text GLabel 8950 2200 2    50   BiDi ~ 0
SDIO_DAT2
Wire Wire Line
	9800 2300 9800 2700
Text GLabel 8950 2300 2    50   BiDi ~ 0
SDIO_DAT3
Text GLabel 13150 4600 2    50   BiDi ~ 0
SDIO_CD
$Comp
L Connector:SD_Card J?
U 1 1 6151133D
P 12150 4800
F 0 "J?" H 12150 5465 50  0000 C CNN
F 1 "SD_Card" H 12150 5374 50  0000 C CNN
F 2 "" H 12150 4800 50  0001 C CNN
F 3 "http://portal.fciconnect.com/Comergent//fci/drawing/10067847.pdf" H 12150 4800 50  0001 C CNN
	1    12150 4800
	1    0    0    -1  
$EndComp
Wire Wire Line
	10600 4500 10600 4700
Wire Wire Line
	10600 4500 10450 4500
Wire Wire Line
	10450 4500 10450 4550
Wire Wire Line
	10600 4700 10600 5000
Connection ~ 10600 4700
Text GLabel 13150 4700 2    50   BiDi ~ 0
SDIO_WP
NoConn ~ 13050 4900
NoConn ~ 13050 5000
Text GLabel 6750 2900 0    50   BiDi ~ 0
SDIO_WP
Text GLabel 6750 3100 0    50   UnSpc ~ 0
GPIO_PB8
Text GLabel 6750 3200 0    50   UnSpc ~ 0
GPIO_PA8
Text GLabel 6750 3300 0    50   UnSpc ~ 0
GPIO_PA12
Text GLabel 6750 4300 0    50   UnSpc ~ 0
GPIO_PD0
Text GLabel 6750 4400 0    50   UnSpc ~ 0
GPIO_PD1
Text GLabel 6750 4500 0    50   UnSpc ~ 0
GPIO_PD2
Text GLabel 6750 4600 0    50   UnSpc ~ 0
GPIO_PD3
Wire Wire Line
	6150 3800 6150 3550
Wire Wire Line
	6050 3550 6050 3650
Text GLabel 6750 3600 0    50   UnSpc ~ 0
GPIO_PB11
Text GLabel 6750 3700 0    50   UnSpc ~ 0
GPIO_PB12
Text GLabel 6750 3500 0    50   Input ~ 0
MCU_RESETN
Text GLabel 11050 1850 3    50   Output ~ 0
MCU_RESETN
Wire Wire Line
	5800 3900 6850 3900
Wire Wire Line
	5500 4200 6850 4200
Wire Wire Line
	6150 3800 6850 3800
Wire Wire Line
	6150 3550 6050 3550
Wire Wire Line
	8950 1600 8850 1600
Wire Wire Line
	8850 1700 8950 1700
Wire Wire Line
	8950 1800 8850 1800
Wire Wire Line
	8850 1900 8950 1900
Wire Wire Line
	8950 2000 8850 2000
Wire Wire Line
	8850 2100 8950 2100
Wire Wire Line
	8950 2200 8850 2200
Wire Wire Line
	8850 2300 8950 2300
Wire Wire Line
	6850 1600 6750 1600
Wire Wire Line
	6750 1700 6850 1700
Wire Wire Line
	6850 1800 6750 1800
Wire Wire Line
	6750 1900 6850 1900
Wire Wire Line
	6850 2000 6750 2000
Wire Wire Line
	6750 2100 6850 2100
Wire Wire Line
	6850 2400 6750 2400
Wire Wire Line
	6750 2500 6850 2500
Wire Wire Line
	6850 2600 6750 2600
Wire Wire Line
	6850 2700 6750 2700
Wire Wire Line
	6850 2800 6750 2800
Wire Wire Line
	6750 2900 6850 2900
Wire Wire Line
	6850 3000 6750 3000
Wire Wire Line
	6750 3100 6850 3100
Wire Wire Line
	6850 3200 6750 3200
Wire Wire Line
	6750 3300 6850 3300
Wire Wire Line
	6850 3400 6750 3400
Wire Wire Line
	6750 3500 6850 3500
Wire Wire Line
	6850 3600 6750 3600
Wire Wire Line
	6750 3700 6850 3700
Wire Wire Line
	6850 4100 6750 4100
Wire Wire Line
	6750 4000 6850 4000
Wire Wire Line
	6850 4300 6750 4300
Wire Wire Line
	6750 4400 6850 4400
Wire Wire Line
	6850 4500 6750 4500
Wire Wire Line
	6850 4600 6750 4600
Wire Wire Line
	6750 4700 6850 4700
Wire Wire Line
	8950 3700 8850 3700
Wire Wire Line
	8850 3800 8950 3800
Wire Wire Line
	8950 3900 8850 3900
Wire Wire Line
	8950 3300 8850 3300
Wire Wire Line
	8850 3200 8950 3200
Wire Wire Line
	8950 3100 8850 3100
Wire Wire Line
	8850 3000 8950 3000
Wire Wire Line
	8950 2900 8850 2900
Wire Wire Line
	11450 1850 11450 1750
Wire Wire Line
	11350 1750 11350 1850
Wire Wire Line
	11150 1850 11150 1750
Wire Wire Line
	11050 1750 11050 1850
Text GLabel 8950 3600 2    50   UnSpc ~ 0
GPIO_PE7
Wire Wire Line
	8950 3600 8850 3600
Wire Wire Line
	8850 3500 9450 3500
Wire Wire Line
	8850 4000 9500 4000
Wire Wire Line
	8850 4100 9100 4100
Wire Wire Line
	8850 4200 9100 4200
Wire Wire Line
	8850 4400 9500 4400
Wire Wire Line
	8850 3400 9950 3400
Text GLabel 8950 4500 2    50   UnSpc ~ 0
GPIO_PD8
Text GLabel 8950 4600 2    50   UnSpc ~ 0
GPIO_PD6
Text GLabel 8950 4700 2    50   UnSpc ~ 0
GPIO_PD5
Wire Wire Line
	8950 4500 8850 4500
Wire Wire Line
	8850 4600 8950 4600
Wire Wire Line
	8950 4700 8850 4700
Text GLabel 11650 3650 3    50   UnSpc ~ 0
GPIO_PD0
Text GLabel 11550 3650 3    50   UnSpc ~ 0
GPIO_PD1
Text GLabel 11450 3650 3    50   UnSpc ~ 0
GPIO_PD2
Text GLabel 11350 3650 3    50   UnSpc ~ 0
GPIO_PD3
Text GLabel 11250 3650 3    50   UnSpc ~ 0
GPIO_PD5
Text GLabel 11150 3650 3    50   UnSpc ~ 0
GPIO_PD6
Text GLabel 11050 3650 3    50   UnSpc ~ 0
GPIO_PD8
Wire Wire Line
	11750 3650 11850 3650
Wire Wire Line
	11850 3650 11850 3750
Wire Wire Line
	11650 3650 11650 3550
Wire Wire Line
	11550 3650 11550 3550
Wire Wire Line
	11450 3650 11450 3550
Wire Wire Line
	11350 3550 11350 3650
Wire Wire Line
	11250 3650 11250 3550
Wire Wire Line
	11150 3550 11150 3650
Wire Wire Line
	11050 3650 11050 3550
Text GLabel 8950 2600 2    50   UnSpc ~ 0
GPIO_PF5
Wire Wire Line
	9600 2400 9600 2450
Wire Wire Line
	8850 2400 9600 2400
Wire Wire Line
	8950 2600 8850 2600
Wire Wire Line
	9450 2500 9450 2700
Wire Wire Line
	8850 2500 9450 2500
Wire Wire Line
	9450 2700 9800 2700
Wire Wire Line
	8850 2800 9950 2800
Text GLabel 8950 2700 2    50   UnSpc ~ 0
GPIO_PF12
Wire Wire Line
	8950 2700 8850 2700
Text GLabel 11450 2950 1    50   UnSpc ~ 0
GPIO_PB8
Text GLabel 11650 2950 1    50   UnSpc ~ 0
GPIO_PA8
Wire Wire Line
	11750 2950 11850 2950
Wire Wire Line
	11850 2950 11850 2850
Text GLabel 11550 2950 1    50   UnSpc ~ 0
GPIO_PA12
Text GLabel 11350 2950 1    50   UnSpc ~ 0
GPIO_PB11
Text GLabel 11250 2950 1    50   UnSpc ~ 0
GPIO_PB12
Text GLabel 11150 2950 1    50   UnSpc ~ 0
GPIO_PE7
Text GLabel 11050 2950 1    50   UnSpc ~ 0
GPIO_PF5
Text GLabel 10950 2950 1    50   UnSpc ~ 0
GPIO_PF12
Wire Wire Line
	11650 2950 11650 3050
Wire Wire Line
	11550 2950 11550 3050
Wire Wire Line
	11450 2950 11450 3050
Wire Wire Line
	11350 2950 11350 3050
Wire Wire Line
	11250 2950 11250 3050
Wire Wire Line
	11150 2950 11150 3050
Wire Wire Line
	11050 2950 11050 3050
Wire Wire Line
	10950 2950 10950 3050
$Comp
L power:+3V3 #PWR?
U 1 1 6171BDCE
P 10750 2850
F 0 "#PWR?" H 10750 2700 50  0001 C CNN
F 1 "+3V3" H 10765 3023 50  0000 C CNN
F 2 "" H 10750 2850 50  0001 C CNN
F 3 "" H 10750 2850 50  0001 C CNN
	1    10750 2850
	1    0    0    -1  
$EndComp
Wire Wire Line
	10850 3050 10850 2950
Wire Wire Line
	10850 2950 10750 2950
Wire Wire Line
	10750 2950 10750 2850
$Comp
L power:GND #PWR?
U 1 1 6172366B
P 10750 3750
F 0 "#PWR?" H 10750 3500 50  0001 C CNN
F 1 "GND" H 10755 3577 50  0000 C CNN
F 2 "" H 10750 3750 50  0001 C CNN
F 3 "" H 10750 3750 50  0001 C CNN
	1    10750 3750
	1    0    0    -1  
$EndComp
Wire Wire Line
	10750 3750 10750 3650
Wire Wire Line
	10750 3650 10850 3650
Wire Wire Line
	10850 3650 10850 3550
Wire Wire Line
	10950 3550 10950 3650
Wire Wire Line
	10950 3650 10850 3650
Connection ~ 10850 3650
NoConn ~ 8850 4300
Wire Wire Line
	9500 4300 9500 4400
$Comp
L Device:C C?
U 1 1 6145F357
P 9500 4150
F 0 "C?" H 9615 4196 50  0000 L CNN
F 1 "1uF" H 9615 4105 50  0000 L CNN
F 2 "" H 9538 4000 50  0001 C CNN
F 3 "~" H 9500 4150 50  0001 C CNN
	1    9500 4150
	1    0    0    -1  
$EndComp
Text Notes 9550 3150 0    50   ~ 0
VREGI/VBUS/VREGO\nAlle knyttet til 5V-regulator\nIkke egt. nødvendig
Wire Notes Line
	13650 5500 13650 900 
Wire Notes Line
	13650 900  3900 900 
Wire Notes Line
	3900 900  3900 5500
Wire Notes Line
	3900 5500 13650 5500
Text Notes 8650 1050 0    50   ~ 0
EFM32GG MCU
Wire Wire Line
	10600 4700 11250 4700
Wire Wire Line
	10300 4800 11250 4800
Wire Wire Line
	10600 5000 11250 5000
Wire Wire Line
	11250 4400 11150 4400
Wire Wire Line
	11150 4500 11250 4500
Wire Wire Line
	11250 4600 11150 4600
Wire Wire Line
	11150 4900 11250 4900
Wire Wire Line
	11250 5100 11150 5100
Wire Wire Line
	11150 5200 11250 5200
Wire Wire Line
	13150 4600 13050 4600
Wire Wire Line
	13050 4700 13150 4700
$EndSCHEMATC
